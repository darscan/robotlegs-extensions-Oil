...
package org.robotlegs.oil.pool
{
	 ...
	 */
	public class RecyclingMediatorMap extends ViewMapBase implements IMediatorMap
	{
		/**
		 * @private
		 */
		 ...
		 */
		public function RecyclingMediatorMap(contextView:DisplayObjectContainer, injector:IInjector, reflector:IReflector)
		{
		...
		public function mapView(viewClassOrName:*, mediatorClass:Class, injectViewAs:* = null, autoCreate:Boolean = true, autoRemove:Boolean = true):void
		{
			...
			config.mediatorPool = new InjectingObjectPool(injector, mediatorClass, null);
			if (injectViewAs)
			{
		...
		public function createMediator(viewComponent:Object):IMediator
		{
			...
			if (mediator == null)
			{
				...
				if (config)
				{
					...
					mediator = config.mediatorPool.get() as IMediator;
		...
		public function removeMediator(mediator:IMediator):IMediator
		{
			if (mediator)
			{
				...
				if (config && mediator is config.mediatorClass)
				{
					config.mediatorPool.put(mediator);
				}
...
class MappingConfig
{
	...
	public var mediatorPool:InjectingObjectPool;
}
