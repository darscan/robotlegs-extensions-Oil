...
package org.robotlegs.oil.pool
{
	 ...
	 */
	public class RecyclingCommandMap implements ICommandMap
	{
		/**
		...
		protected var commandClassPoolsByClassName:Dictionary;
		 ...
		 */
		public function RecyclingCommandMap(eventDispatcher:IEventDispatcher, injector:IInjector, reflector:IReflector)
		{
			...
			this.commandClassPoolsByClassName = new Dictionary(false);
		}
		...
		public function execute(commandClass:Class, payload:Object = null, payloadClass:Class = null, named:String = ''):void
		{
			...
			var commandClassName:String = reflector.getFQCN(commandClass);
			var pool:InjectingObjectPool = commandClassPoolsByClassName[commandClassName];
			if (!pool)
			{
				pool = new InjectingObjectPool(injector, commandClass, null);
				commandClassPoolsByClassName[commandClassName] = pool;
			}
			
			var command:Object = pool.get();
